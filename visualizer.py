...
def generate_charts(sentiment, pos_keywords, neg_keywords):
    ...
    ax1.text(0, 0, f'Total\n{total_reviews}', ha='center', va='center', fontsize=14, fontweight='bold', color='#2C3E50')
    ax1.set_title("Sentiment Distribution", fontsize=14, fontweight='bold', pad=20, color='#2C3E50')
    ...
    bars = ax2.barh(y_pos, counts, color=colors_bar, height=0.6, edgecolor='#ffffff', alpha=0.9)
    ...
    for bar in bars:
        ...
        ax2.text(width + 0.3, bar.get_y() + bar.get_height()/2, f'{int(width)}', 
                 ha='left', va='center', fontsize=10, color='#555555', fontweight='bold')
    ...
    ax2.set_title("Keyword Mentions (Green=Pos, Red=Neg)", fontsize=14, fontweight='bold', pad=20, color='#2C3E50')

    plt.tight_layout()

    # Save chart to a memory buffer
    img_buf = io.BytesIO()
    plt.savefig(img_buf, format='png', dpi=150, bbox_inches='tight', facecolor=fig.get_facecolor())
    img_buf.seek(0)
